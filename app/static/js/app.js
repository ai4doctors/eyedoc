...
function renderDx(){
  ...
  dx.forEach(item => {
    ...
    // ICD codes shown on hover only
    const icd10 = item.icd10_code || item.code || ""
    const icd9 = item.icd9_code || ""
    if(icd10 || icd9){
      const tooltip = document.createElement("div")
      tooltip.className = "icdTooltip"
      let tooltipContent = ""
      if(icd9){
        tooltipContent += `<span class="icdLabel">ICD-9:</span> ${icd9}`
      }
      if(icd10){
        if(icd9) tooltipContent += " &nbsp;|&nbsp; "
        tooltipContent += `<span class="icdLabel">ICD-10:</span> ${icd10}`
      }
      if(item.icd10_description){
        tooltipContent += ` <span style="color:var(--muted);font-style:italic">(${item.icd10_description})</span>`
      }
