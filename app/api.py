...
def pubmed_fetch_for_terms(terms: List[str], max_items: int = 12, patient_age: Optional[int] = None, detected_specialty: str = "") -> List[Dict[str, str]]:
    """Return empty list - we only use verified canonical references now.
    
    The PubMed API can return irrelevant or low-quality results.
    We rely exclusively on the curated canonical_reference_pool instead.
    """
    return []
